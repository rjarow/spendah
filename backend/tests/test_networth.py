...
class TestNetWorthCalculation:
    ...
    def test_get_current_networth_single_asset(self, db_session):
    ...
    def test_get_current_networth_single_liability(self, db_session):
    ...
    def test_get_current_networth_mixed(self, db_session):
...
class TestNetWorthBreakdown:
    ...
    def test_get_networth_breakdown_single_account(self, db_session):
...
class TestBalanceHistory:
    ...
    def test_record_balance_snapshot(self, db_session):
    ...
    def test_record_balance_snapshot_default_date(self, db_session):
        ...
        snapshot = record_balance_snapshot(db_session, account.id, Decimal("1000.00"))

        assert snapshot.recorded_at == date.today()

    def test_record_balance_snapshot_invalid_account(self, db_session):
        """Record balance snapshot for non-existent account."""
        with pytest.raises(NetWorthError):
            record_balance_snapshot(db_session, "non-existent-id", Decimal("1000.00"))

    def test_record_balance_snapshot_different_dates(self, db_session):
    ...
    def test_auto_snapshot_all_balances(self, db_session):
        ...
        result = auto_snapshot_all_balances(db_session)

        assert result["total_snapshots"] == 2
        assert result["errors"] == 0

        # Verify snapshots were created
        snapshots = db_session.query(BalanceHistory).all()
...
class TestNetWorthHistory:
    ...
    def test_get_networth_history_empty(self, db_session):
        """Get history when no snapshots exist."""
        history = get_networth_history(db_session, date(2024, 1, 1), date(2024, 12, 31))

        assert history == []

    def test_get_networth_history_with_snapshots(self, db_session):
    ...
    def test_get_networth_history_single_date(self, db_session):
    ...
    def test_get_networth_history_no_data_range(self, db_session):
    ...
    def test_get_networth_history_mixed_accounts(self, db_session):
